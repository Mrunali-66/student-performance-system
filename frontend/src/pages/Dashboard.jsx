/**
 * src/pages/Dashboard.jsx  [TEACHER ONLY — legacy, kept during migration]
 *
 * Fixed:
 *  - Changed /students → /teacher/students (teacher-scoped endpoint)
 *  - Changed /analysis  → /teacher/analytics (teacher-scoped endpoint)
 *  - Import fixed to '../utils/api'
 *  - Added useAuth import for teacher context
 */
...
function SBar({ score }) {
  ...
  return (
    <div className="sbar">
      <div className="sbar-track"><div className="sbar-fill" style={{ width: `${p}%`, background: col }} /></div>
      <span style={{ fontSize: 11, color: 'var(--text3)', minWidth: 28, fontFamily: 'var(--mono)' }}>{p.toFixed(0)}</span>
    </div>
  )
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // FIXED: teacher-scoped endpoints
    Promise.all([
  ...
  const avgScore = students.length
    ? (students.reduce((s, st) => s + (st.performance_score || st.predicted_performance || 0), 0) / students.length).toFixed(1) : 0
  const passRate = students.length
    ? Math.round(students.filter(s => (s.performance_score || s.predicted_performance || 0) >= 50).length / students.length * 100) : 0
  const avgAtt   = students.length
    ? Math.round(students.reduce((s, st) => s + (st.attendance || 0), 0) / students.length) : 0
  ...
  return (
    <div className="page">
      <div className="page-header fu">
        <h1>Dashboard</h1>
        <p>Overview of your student performance metrics</p>
      </div>
      {error && <div className="error-banner">⚠ {error}</div>}

      <div className="stats-grid">
        {[
          { cls: 'c-cyan', icon: '👥', lbl: 'Total Students',  val: analysis.total || students.length || 0,  sub: 'Enrolled this term', link: '/students', ll: 'View all →' },
          { cls: 'c-red',  icon: '⚠',  lbl: 'At Risk',         val: analysis.weak || 0,                      sub: 'Need intervention',  link: '/weak-students', ll: 'View all →' },
      ...
      <div className="card fu">
        <div className="card-hdr">
          <div>
            ...
            <div className="card-sub">Latest entries — click Analyse for individual insights</div>
          </div>
        ...
        {recent.length === 0 ? (
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Student</th><th>Batch</th><th>Attendance</th>
                  <th>Study Hrs</th><th>Performance</th><th>Risk</th><th>Analysis</th>
                </tr>
              </thead>
              <tbody>
                {recent.map(s => (
